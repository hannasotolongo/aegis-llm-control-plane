...
func TestRiskAwareServiceChangesPlacementFromBaseline(
	t *testing.T,
) {
	...
	results.Set(predictor.Result{
		Forecast: predictor.Forecast{
			...
			PredictedComputeUtilization: 70,
	...
	if placed[0].AssignedWorkerID != "worker-2" {
		t.Fatalf(
			"expected risk-aware placement on worker-2, got %s",
